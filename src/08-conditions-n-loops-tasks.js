...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const ch = str[i];
    if (str.indexOf(ch, i + 1) === -1 && !seen.has(ch)) {
      return ch;
    }
    seen.add(ch);
  }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${open}${smaller}, ${larger}${close}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  function computeCheck(num) {
    const digits = [];
    const digs = num;
    for (let i = 0; i < digs.length; i += 1) {
      let d = +digs[digs.length - 1 - i];
      if (i % 2 === 0) {
        d *= 2;
      }
      digits.push(digitSum(d));
    }
  ...
  const testCheck = +testData.slice(-1);
  const computedCheck = computeCheck(testData.slice(0, -1));
  return testCheck === computedCheck;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openers.indexOf(ch) !== -1) {
      ...
      const op = stack.pop();
      if (openers.indexOf(op) !== closers.indexOf(ch)) {
        return false;
      }
    }
  }
